...
```
One install brings the whole suite (7 skills + 10 commands: `/omg` + 9 `/omg:*`). There are no separate/optional plugins to add.
...
for skill in no-english workflow-eta easy-answer plain-layer branch-flow worktree gjc-bugwatch multivendor-presets release-gate; do
  test ! -e "$root/skills/$skill" && test ! -L "$root/skills/$skill" || exit 1
done
for retired in easy easy-always plain branchflow-always worktree bugwatch-scan presets release no-english; do
