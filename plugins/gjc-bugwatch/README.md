...
정밀도 노트:
- **로그 = 고정밀** (gjc의 구조화 로그, 오탐 사실상 0). 롤오버된 **`*.log.gz`도 읽음**(어제 이전 히스토리 포함).
- **stale 표시** — 마지막 발생이 `--fresh-days`(기본 2일)보다 오래된 후보는 `⏳stale`로 찍고 맨 아래로 내린다(이미 upstream에서 고쳐졌을 확률↑). 죽은 버그 쫓기 방지. `--fresh-only`로 숨김.
