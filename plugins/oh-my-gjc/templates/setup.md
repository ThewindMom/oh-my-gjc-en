...
설치는 번들 파일을 네이티브로 복사하고, workflow-eta의 exact-lock SDK runtime을 전제조건이
...
if command -v bun >/dev/null 2>&1 && [ -f "$workflow_eta_runtime/src/eta.ts" ]; then
  ...
  printf '%s\n' 'workflow-eta SDK runtime unavailable — rerun hardened installer with Bun >=1.3.14'
fi
```
...
- 0.19.0 업그레이드는 이름이 바뀐 `gate-briefing` 디렉터리를 제거하고 `adaptive-response`를 설치한다.
  `workflow-eta` SDK runtime은 exact lockfile로 원자적으로 교체하며 실패하면 스킬을 fail-closed로 둔다.
