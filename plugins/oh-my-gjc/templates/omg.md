...
## 스킬 (7)
- `adaptive-response`(`/omg:gate*`에서만 명시 호출) · `no-english`(`/omg:no-english`에서만 명시 호출) · `time-left`(`/omg:time-left`에서만 명시 호출) · `extragoal`(외부 최종 리뷰 게이트) · `insane-review` · `lazycodex-gjc`(읽기 전용) · `deep-onboarding`(분석·인터뷰 후 확인된 경로에만 문서 생성)
