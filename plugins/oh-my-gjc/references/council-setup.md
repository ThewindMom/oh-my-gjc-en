...
council worker는 셸 없이 execFile 하므로 `command`엔 **엔진의 절대경로**를 넣는다(공백 없게). gjc 설치본 경로 확인:
```bash
ls -d ~/.gjc/plugins/cache/plugins/oh-my-gjc___oh-my-gjc___*/bin/pack_and_ask.py 2>/dev/null | sort -V | tail -1
# 또는 이 마켓플레이스 레포에서: <repo>/plugins/oh-my-gjc/bin/pack_and_ask.py
```
...
python3 "$IR" --council --model pro --require-model "GPT-5.5" \
