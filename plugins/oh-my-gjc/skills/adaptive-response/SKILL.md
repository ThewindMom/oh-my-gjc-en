...
description: 현재 대화·인터뷰·작업에서 확인된 사용자의 도메인 숙련도·설명 선호·의사결정 역할로 일반 답변과 GJC 스킬 응답을 근거 기반으로 보정하고, 승인 게이트에서는 같은 수준에 맞춘 4부 브리핑을 제공한다. "내 수준에 맞춰 설명해", "인터뷰 답변을 반영해", "기초 설명은 빼줘", "쉽게/깊게 설명해", "승인해도 돼?", "이 계획 무슨 뜻이야" 같은 요청과 계획 승인 직전 상황에서 활성화한다. 임시 응답 페르소나는 현재 도메인에만 적용하며 정확성·안전·승인 경계를 절대 낮추지 않는다.
---
...
- 사용자가 "내 수준에 맞춰", "기초 설명은 빼줘", "쉽게/깊게 설명해"처럼 응답 수준 보정을 요청할 때
...
- 스킬이 자동 활성화되거나 사용자가 수준 보정을 요청한 현재 응답에는 이 절차를 적용한다.
