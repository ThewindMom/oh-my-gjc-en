...
- runner는 호환 OMO 4.x `ultrawork`(최소 4.18.0)를 읽기 전용으로 검증·고정한다. custom Codex permission profile로 target `cwd`, 정확한 Codex runtime helper, private tmp만 노출하고 web/MCP/apps/hooks/browser egress와 child shell 환경 상속을 끈다.
