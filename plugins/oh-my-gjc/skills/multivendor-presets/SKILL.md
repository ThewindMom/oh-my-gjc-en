...
4. 은퇴 프리셋 블록(닫힌 목록 — `/omg:presets` 커맨드 본문의 구버전 정리 목록과 동일: `ultimate`/`ultimate-f5`/`daily`/`fast`/`ideal`/`escalate-surgical`/`monorepo`/`reviewer`/`fable-sol`/`grok-main`/`grok`/`codex`/`fable-codex`)이 보이면 사용자 동의 후에만 제거. 목록 밖 프로파일은 절대 제거 금지.
