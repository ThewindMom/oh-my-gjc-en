...
describe("time-left public skill", () => {
  ...
  test("requires the explicit slash command", () => {
    ...
    expect(read(installer)).toContain("EXPECTED_COMMANDS=(omg setup gate gate-always no-english time-left fable insane-review lazycodex-gjc deep-onboarding)");
  ...
  test("pins the official bridge client and installer runtime contract", () => {
    ...
    expect(installScript).toContain("EXPECTED_SKILLS=(adaptive-response no-english time-left extragoal insane-review lazycodex-gjc deep-onboarding)");
