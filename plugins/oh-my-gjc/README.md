...
**Gajae Code(gjc)의 oh-my 단일 플러그인.** 한 번 설치로 스킬 5개 + 커맨드 7개
...
### 스킬 (자연어 트리거 자동활성화)
`adaptive-response` · `workflow-eta` · `extragoal` · `insane-review` · `lazycodex-gjc`
...
`gjc-bugwatch`)의 네이티브 잔재를 정리한다. `adaptive-response`와 `workflow-eta`를 설치하고,
