...
**Gajae Code(gjc)의 oh-my 단일 플러그인.** 한 번 설치로 스킬 4개 + 커맨드 7개
(`/omg` + `/omg:*` 6개)가 전부 들어온다. `insane-review`는 ChatGPT+크로미움,
...
### 스킬 (자연어 트리거 자동활성화)
`gate-briefing` · `extragoal` · `insane-review` · `lazycodex-gjc`
...
`easy-answer`, `plain-layer`, `branch-flow`, `gjc-bugwatch`)의 네이티브 스킬·커맨드
잔재를 정리한다. `lazycodex-gjc`는 유지되며, 런타임 전제조건이 없을 때만 stale binding을
제거해 fail-closed 상태로 둔다. 기존 `models.yml`은 자동으로 수정하지 않는다.
### 가재 앱 마이그레이션 (0.14.0)
