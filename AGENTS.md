...
- **gjc ignores plugin skill surfaces** (`skills.ts`: `if (provider !== "native") return false` — only native `.gjc` skills load). So after `gjc plugin install`, the **command** works but the plugin's `SKILL.md` does NOT auto-activate / show in the `skill` tool. To get the skill surface, run `bin/install-skill.sh` (copies SKILL.md into `~/.gjc/agent/skills/insane-review/` for user scope, or `<cwd>/.gjc/skills/` for project). Applies to every marketplace plugin, not just this one.
...
### `multivendor-presets` (static + dry-run verified)
- Command `/multivendor-presets:install [ultimate|ultimate-f5|all]` + native-installable skill (`skills/multivendor-presets/SKILL.md`) + canonical `references/presets.yml`. Installs **multi-vendor model-profile presets** into `~/.gjc/agent/models.yml` `profiles:` so `gjc --mpreset <name>` can activate them. gjc plugin manifests have **no `profiles` field**, so the plugin cannot auto-inject profiles — the command drives gjc to do a **name-scoped safe merge** instead.
- **Two presets** (both `required_providers: [anthropic, openai-codex, google-antigravity, xai]`): `ultimate` (cost-no-object, best model per role — executor Opus:max, planner GPT-5.5:xhigh, architect Gemini 3.1 Pro, critic Grok 4.3) and `ultimate-f5` (the user's `escalation` profile **renamed** per request: router/`default` stays on Opus 4.8:high for stability, only `executor` escalates to `anthropic/claude-fable-5:xhigh` as a relief pitcher). Source of truth for the mappings is the user's own `~/.gjc/agent/models.yml`.
- **Merge safety contract (do not weaken):** name-scoped only (replace the target profile block if present, else append under `profiles:`); never delete/modify other profiles or top-level keys (`default`, `modelBindings`, …); back up to `.bak-<ts>` first; result must be valid YAML with the target present or roll back (no partial save); preserve 2/6-space indentation + `required_providers`/`model_mapping` structure + original comments. The comment-preserving merge is agent-driven (via `read`/`edit`) rather than a PyYAML round-trip, which would drop the Korean rationale comments.
- **⚠ `ultimate-f5` executor must be `:xhigh`, not `:max`** — Fable 5 silently clamps `:max` down to `xhigh`. Activation hard-blocks when any of the 4 providers lacks credentials (verified at `--mpreset` time, not at merge time). Selectors/prices are catalog- and time-sensitive (2026-06~07); re-verify with `gjc -p --no-session --no-tools --model <selector> "Reply OK"`.
- Non-Goals: vendor auto-login (`/login` is manual), profile auto-injection (manifest unsupported — merge is explicit-command only), selector/price freshness guarantees.

### `gjc-bugwatch` (collector verified on real logs; triage/PR is agent-driven)
- Dogfooding bug collector. `bin/collect.ts` (batch scanner) reads `~/.gjc/logs/*.log` (structured JSONL: `{level,message,stack,...}`) and optionally session transcripts (`~/.gjc/agent/sessions/--<cwd>--/*.jsonl`), classifies signals (`gjc-internal`/`error`/`warn`), filters env/credential noise, dedupes by normalized fingerprint, and redacts email/UUID/token/creds-in-URL. `/gjc-bugwatch:scan` + skill drive triage → reproduce against a `/tmp` gajae-code clone → collect issue/PR **drafts** under `.gjc/bugwatch/drafts/`.
...
### `my-workflows`
- `easy-answer` skill (rephrase final answers in plain language, **accuracy-first** — keep the technical term when simplifying would distort) + three surfaces:
  - `/my-workflows:easy [on|off]` — this-session toggle.
  - `/my-workflows:easy-always [on|off|status]` — **always-on** across every session by writing a delimited rule block (`<!-- BEGIN/END my-workflows:easy-always -->`) into user-global `~/.gjc/agent/AGENTS.md`, which gjc injects into every turn's context. This is the correct "매번 자동" mechanism: gjc does NOT auto-load plugin `SKILL.md` (native `.gjc` skills only), and plugin hooks are constrained (declared tool_call event only; session-mutation/context-injection denied), so neither can force plain language every turn — a context-file block can.
  - `bin/install-skill.sh` — native-install the `easy-answer` skill (→ trigger-word activation like "쉽게 말해줘" + `/skill`). Relevance-triggered, not every-turn; for every-turn use `easy-always on`.
- Block-merge safety (easy-always): owns ONLY the marker block; backs up `~/.gjc/agent/AGENTS.md` to `.bak-<ts>` first; never touches content outside the markers; exactly one block (replace on re-run, clean removal on `off`).
