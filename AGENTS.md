...
- **Single bundled engine** (`bin/pack_and_ask.py`, Playwright-based, cross-platform). It owns packing, the headless-by-default/visible-login browser lifecycle, CDP automation, and fail-closed retrieval. The gjc shell adapts the skill/command to gjc terms + `ask` onboarding; the Claude-Code `setup/` (GitHub-star prompt + `~/.claude/settings.json` SessionStart update hook) remains **dropped**. Do not reimplement the engine flow with gjc's `browser` tool.
...
- **Prerequisites (manual):** Python `playwright`+`pyperclip` (`--check-env --install`), Node/`npx` (repomix auto via `npx -y`), and a Chromium-family browser on CDP `:9222` with a **dedicated persistent profile** logged into chatgpt.com + GPT-5.6 Sol Pro selected. Automation launches headless by default. If authentication is missing, `--launch-browser-visible` restarts that same profile only for interactive login; confirmation returns it to headless mode. Login can't be automated.
