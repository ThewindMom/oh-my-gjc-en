...
- `adaptive-response`, `extragoal`, and the `example-plugin` template: no external prerequisites. `workflow-eta` requires Linux, Bun >=1.3.14, its exact-lock private SDK runtime, and a live top-level GJC SDK endpoint.
