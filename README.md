...
- `sol` — 전 구간 저지연: default `sol:low`, 기획 좌석도 빠름(planner `sol:high` · architect `opus:medium` · critic `opus:high`). ralplan이 빨리 돌아야 할 때 이걸 쓴다. (executor는 벤치 근거 `terra:xhigh` 유지.)
